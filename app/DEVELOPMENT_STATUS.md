...
- 初始化时通过固定主机指纹的 Key-only SSH 会话检测远端端口，并自动选择、持久化可用端口。
...
- 已提供 Windows x64 自包含单文件发布配置和 `publish-portable.ps1`：复制运行脚本/手册、排除个人 `config.local.json` 与 PDB、生成 EXE/ZIP SHA256，并在发布目录及 ZIP 独立解压目录执行无界面包自检。
...
- PowerShell 的 `setup`/`start` 会先安装 SSH 别名，再进行 Key 登录检查。
...
Real SSH status:   passed in 6 seconds (proxy running, tunnel stopped, key ready)
Embedded UI:       passed (all surfaces, 63 guide blocks, proxy status, bounded process-output drain)
...
6. 生成自包含 Windows 发行包，补充应用图标、版本信息、代码签名与升级策略。
