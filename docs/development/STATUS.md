...
- GUI 当前服务器操作行提供“一键修复连接”；应用层复验失败时不再无条件报告成功。活动 Codex 路线陈旧会显示橙色“隧道正常 · 需重载 VS Code”、采用进程数和中文下一步，并仅在用户确认后重启远程 VS Code Server、重新打开项目及复验。
- 多服务器异步工作流固定绑定 Profile ID、配置路径和服务器标识；操作期间锁定选择框，切换后的旧结果会被丢弃并自动检查当前服务器，日志明确标出结果归属。
- 服务器页使用按连接状态着色的摘要卡突出显示 Profile 归属、中文结论和建议操作；原始 PID、网络标记等技术输出保留在默认折叠的详情区域，避免内部标记淹没用户需要看到的结果。
- GUI 诊断优先级固定为 SSH → 本机代理/隧道 → 远程应用网络 → Codex 登录，避免下游探针失败被误报成“需要登录 Codex”。
- 直连与代理应用复验只接受 Codex 实际端点对无凭据 POST 返回的 `401`，不再用普通公网探针或其他 4xx 状态替代 Codex 可用性。
- 直连选择、代理健康检查和应用层复验均进行一次短时重试，连续失败后才切换或报告路线不可用。
- 远端 `~/.bashrc` 写入统一转换为 LF，清理逻辑兼容历史 CRLF 标记并在替换前执行 `bash -n`；状态与诊断新增应用层代理检查，避免隧道进程存活但 VS Code/Codex 未继承代理时误报绿色。
- 连接、修复和初始化现已使用 Codex 实际端点执行服务器直连探测：直连可用时选择 `direct` 且不新建隧道，失败时自动选择 `proxy`；状态和 GUI 明确显示当前应用网络路由。
