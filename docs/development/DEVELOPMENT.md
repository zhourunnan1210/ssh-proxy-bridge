...
5. 初始化成功后，点击“连接并打开 VS Code”。应用会建立反向隧道、验证服务器代理、启动独立自动修复监控、写入受管的远程 Shell 代理块，并打开配置的远程目录。
...
- “运行诊断”和“一键修复连接”位于当前服务器的操作按钮行，与“刷新状态”“停止连接”处于同一层级；操作行使用可换行布局适配最小窗口宽度。
- “一键修复连接”只操作通过 Profile PID、进程类型、SSH 别名和转发参数归属校验的进程；健康时不重建，失效时修复，并确保后台监控运行。只有活动 Codex 仍使用旧路线时，才会在用户明确确认后重载远程 VS Code。
- 每个异步工作流固定绑定启动时的 Profile ID 和运行配置；执行期间服务器选择框不可操作。结果返回时若当前选择已变化，旧结果必须丢弃并对当前 Ready Profile 重新执行只读状态检查。
...
- 停止隧道前同时核对 PID、`ssh.exe`、SSH 别名和远端代理端口；停止后台监控前核对 PID、PowerShell 进程、脚本路径、`monitor` 命令和 Profile 配置路径。
- 每个 Profile 使用独立修复锁，避免 GUI 和后台监控同时创建重复隧道；失败重试按 2、5、10、30、60、300 秒退避。
...
当前基线：Core 测试 `18/18` 通过；界面冒烟测试已验证添加、编辑、删除、密码、通知和使用说明均在主窗口内呈现，并验证代理状态、修复入口、自动修复状态、原位导航与 AskPass 凭据读取；Release 构建 `0` 错误。迁移测试验证旧数据只复制不删除，凭据测试只创建随机虚拟凭据，并在 `finally` 中立即删除。
