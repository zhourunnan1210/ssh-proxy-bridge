...
用户首次添加服务器时，应用通过向导收集代理和 SSH 信息，验证服务器身份，选择性地将服务器密码保存到 Windows 凭据管理器，安装专用 SSH 公钥并配置远程代理环境。完成首次配置后，用户只需点击“连接并打开 VS Code”，应用即可启动代理、建立隧道、验证网络并打开指定的远程目录。
...
日常一键连接使用专用 SSH Key。原因是 VS Code Remote-SSH 使用本机 OpenSSH；把保存的密码自动注入其交互提示会引入兼容性和泄露风险。密码认证仍可作为手动回退，但不作为第一版的无感日常链路。
...
| FR-043 | P0 | 安装后必须通过 `BatchMode=yes` 验证 Key-only 登录，验证失败则首次配置不算完成。 |
...
- 不通过命令行参数、环境变量、临时文件、标准输出或异常消息传递密码。
...
选择 SSH.NET 的目的不是取代 VS Code 的 OpenSSH，而是安全完成密码认证、公钥安装、远程命令和必要的反向转发控制，避免把密码放进子进程参数。日常 VS Code 仍通过标准 SSH Config 和 Key 连接。
...
2. `OpenSshTunnelProcess`
   - 使用 Windows `ssh.exe` 和专用 Key 维持日常隧道。
...
4. 能生成并安装专用 SSH Key，且验证 Key-only 登录。
